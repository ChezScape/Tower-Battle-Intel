...
export function buildPrimaryStatGrid(state = {}) {
    return `
        <div class="tbi-grid tbi-grid-4 tbi-primary-matrix">
            ...
            ${buildMetricTableCard("Defense & Survival", mergeSections(state.sections, ["damage_taken", "health_regenerated", "damage_blocked"]), { icon: "⬡", accent: "blue", limit: 6, footer: "View Full Defense Breakdown", target: "defense" })}
...
export function buildSecondaryStatGrid(state = {}) {
    return `
        <div class="tbi-grid tbi-grid-4 tbi-secondary-matrix">
            ...
            ${buildMetricTableCard("Effects Active", state.sections.killed_with_effect_active, { icon: "✦", accent: "pink", limit: 5, footer: "View Effect Breakdown", target: "effects" })}
        </div>
    `;
}
...
export function buildMetricTableCard(title, section, { icon = "◇", accent = "cyan", limit = 8, footer = "View Details", target = "" } = {}) {
    return `
        <section class="tbi-card tbi-metric-card ${escapeAttr(accent)}">
            ...
            <button type="button" class="tbi-card-footer-action" data-ui-action="open-compare-section" data-section-target="${escapeAttr(target || title)}">${escapeHTML(footer)}</button>
        </section>
    `;
}
